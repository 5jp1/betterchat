...
import type { Room, User, FriendsData, Announcement, SystemState } from '@/lib/definitions';
import { cn } from '@/lib/utils';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/components/ui/tooltip';
import Image from 'next/image';
import { Button } from '../ui/button';
import { UserAvatar } from './user-avatar';
import { LogOut, MessageSquare, Hash, AtSign, Settings, Plus, X, Trash2, Inbox, UserPlus, Terminal, Frown } from 'lucide-react';
...
export function ChatLayout({ rooms, allUsers, friendsData, children, announcements, systemState }: ChatLayoutProps) {
  ...
  if (systemState?.isShutdown && user?.role !== 'admin') {
      return (
          <div className="flex h-screen flex-col items-center justify-center bg-background text-foreground">
              ...
              <p className="mt-2 text-muted-foreground">BetterChat is currently undergoing maintenance. Please check back later.</p>
          </div>
      );
  }
  ...
  return (
    <TooltipProvider>
      ...
      {isAdmin && <AdminPanelDialog open={isAdminPanelOpen} onOpenChange={setAdminPanelOpen} allUsers={allUsers} systemState={systemState} />}
        ...
        <main className="flex flex-1 flex-col bg-background">
            <header className="flex h-12 flex-shrink-0 items-center justify-between border-b border-border px-4 shadow-sm">
                <div className="flex items-center text-lg font-semibold">
                   ...
                   <span>{isDM ? allUsers.find(u=> u.id === activeChatId)?.username : currentRoom?.channels?.[0]?.name}</span>
                </div>
                {isAdmin && (
                    <AlertDialog>
                        <Tooltip>
